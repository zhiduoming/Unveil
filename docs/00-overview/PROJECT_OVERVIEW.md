...
> **目标读者**：课程验收教师（约 3 分钟了解全貌）  
> **统计基准**：5 个 Maven 模块 · 63 个 Java 源文件（主代码）· ~7,540 行（`scripts/count-loc.ps1` 实测；含测试共 114 文件 / ~10,540 行）
...
| 类别 | 文档 |
|---|---|
| 功能完成度 | [FEATURE_MATRIX.md](./FEATURE_MATRIX.md) |
| 术语表 | [GLOSSARY.md](./GLOSSARY.md) |
| 需求追踪 | [../01-requirements/REQUIREMENTS.md](../01-requirements/REQUIREMENTS.md) |
| 总体架构 | [../02-design/ARCHITECTURE.md](../02-design/ARCHITECTURE.md) |
| 规则引擎 | [../02-design/RULE_ENGINE_DESIGN.md](../02-design/RULE_ENGINE_DESIGN.md) |
| AI 算法 | [../02-design/AI_DESIGN.md](../02-design/AI_DESIGN.md) |
| 复盘设计 | [../02-design/REPLAY_DESIGN.md](../02-design/REPLAY_DESIGN.md) |
| 协议规范 | [INTERFACE.typ](../INTERFACE.typ) |
| 构建运行 | [../04-deployment/BUILD_AND_RUN.md](../04-deployment/BUILD_AND_RUN.md) |
| 测试用例 | [../05-testing/TEST_CASES.md](../05-testing/TEST_CASES.md) |
| 测试报告 | [../05-testing/TEST_REPORT.md](../05-testing/TEST_REPORT.md) |
| 演示脚本 | [../07-presentation/DEMO_SCRIPT.md](../07-presentation/DEMO_SCRIPT.md) |

---

## 项目名称与代号

**Unveil**（揭棋对弈程序）— 北京邮电大学「揭棋对弈程序设计」课程大作业，第一组（组长：张恒基）。
...
| 代码行数 | **~7,540** | 主代码 LOC（`count-loc.ps1` 实测 7,539） |
| 含测试合计 | **114 文件 / ~10,540 行** | `count-loc.ps1 -IncludeTests` |
...
| 方式 | 命令 / 脚本 | 预期行为 |
|---|---|---|
| **一键演示（推荐）** | `powershell -File scripts/demo.ps1` | 自动打开 3 个窗口：WS 服务器（8887）+ 客户端 player1 / player2 |
| 交互菜单 | `mvn exec:java -f jieqi-app/pom.xml -am` | 数字菜单：服务器 / 客户端 / 人机 / AI 自动对弈等 |
| Fat JAR | `java -jar jieqi-app/target/unveil-jieqi.jar server-ws 8887` | 打包后单 JAR 启动 |

**演示截图说明**：客户端连接成功后显示 10×9 棋盘（`?` 表示暗子）；双方 `match` → `ready` → `first` 开局后可 `move a 6 a 5` 走子；终局后可 `replay` 逐步回看（`n`/`p`/`g 步号`）。
...
| 课程要求 | 本项目实现 | 验收入口 |
|---|---|---|
| 网络揭棋双人对弈 | `WsGameServer`（8887）+ `WsGameClient` | `scripts/demo.ps1` 或双客户端 `match` |
| 服务器校验非法着法 | `Game.processMove` → `RuleValidator` | 发送非法坐标，收到 `moveResult.valid=false` |
| 棋谱记录 | `GameRecord` + `GameRecordStore` → `records/*.jieqi` | 对局结束查看 `jieqi-server/records/` |
...
| 面向对象设计 | 领域类 `Board`/`Game`/`ChessPiece` 与模块分层 | [DOMAIN_MODEL.md](../02-design/DOMAIN_MODEL.md) |
| 文档与组间互操作 | `docs/03-interface/INTERFACE.typ` v3.0 | 与公共 JSON 消息类型对齐 |
