...
> **目标读者**：老师、联调同学、答辩听众  
> **关联**：[PROJECT_OVERVIEW.md](./PROJECT_OVERVIEW.md) · [DOMAIN_MODEL.md](../02-design/DOMAIN_MODEL.md)
...
| 将帅照面 | Kings Facing | 将帅同列无子阻隔，规则禁止形成 | `RuleValidator.isInCheck` 照面分支 |
...
| Aspiration Window |  aspiration 窗口 | 以上次分数为中心缩窄 α-β 窗口 | `OptimizedAlphaBeta` |
| LMR | Late Move Reduction | 非杀手/非历史走法减深搜索 | `OptimizedAlphaBeta` |
| 课程公共接口 | Teacher Protocol | 2026 大作业 WebSocket JSON 必做消息 | `docs/INTERFACE.typ` 正文 |
| 本组扩展 | Team Extension | rematch / replay / addTime 等扩展消息 | `JsonMessageTypes` 注释区 |

---

*每行 1–2 句定义；详细规则见 [RULE_ENGINE_DESIGN.md](../02-design/RULE_ENGINE_DESIGN.md)*
